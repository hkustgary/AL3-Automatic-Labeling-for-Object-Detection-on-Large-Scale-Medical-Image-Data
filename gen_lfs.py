...
def create_datasets(root_dir, train_size=0.05):
    ...
    train_indices, val_indices = train_test_split(
        ...
        train_size = 0.05, # use 5% training data
        test_size=0.2,
        random_state=42
    )
...
def train_lf(type ='faster-rcnn'):
    ...
    torch.save(model.state_dict(), 'lfs/' + type + '.pth')
    return best_performance_map

def start_gen():
    if os.path.exists('lfs/faster.pth'):
        if os.path.exists('lfs/fcos.pth'):
            if os.path.exists('lfs/retina.pth'):
                print('LFs already exists.')
                return
    print('start generating LFs')
    train_lf('faster')
    train_lf('fcos')
    best_performance_map = train_lf('retina')
    print('finish generating LFs')
    print('retina with 0.05 data, best performance: ' + str(best_performance_map))
